...
- Releases are CI-automated: `.github/workflows/release.yml` builds deb/rpm (Linux), NSIS/MSI (Windows), DMG (macOS Apple Silicon) on every `v*` tag; the release is a draft until published manually.
